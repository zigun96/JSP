	...
	public boolean insertMember(MemberBean bean) {
		...
		try {
			...
			sql = "insert tblMember(id,pwd,name,gender,birthday,email,zipcode"
					+ ",address,hobby,job,nation)values(?,?,?,?,?,?,?,?,?,?,?)";
			pstmt = con.prepareStatement(sql);
			...
			for (int i = 0; i < hobby.length; i++) {
				...
				for (int j = 0; j < lists.length; j++) {
					// 저장된 취미와 실제 항목 일치시 '1' 대입
					if (hobby[i].equals(lists[j]))
						hb[j] = '1';
						System.out.println(hobby[i]);
						System.out.println(lists[j]);
						System.out.println(hb[j]);
				}
			}
			pstmt.setString(9, new String(hb));
			pstmt.setString(10, bean.getJob());
			pstmt.setString(11, bean.getNation());
	...
	public MemberBean getMember(String id) {
		...
		try {
			...
			if (rs.next()) {
				...
				String hobby = rs.getString("hobby");// 01001 : 0(비 체크) 1(체크)
	...
	public boolean updateMember(MemberBean bean) {
		...
		try {
			...
			String sql = "update tblMember set pwd=?, name=?, gender=?, birthday=?,"
					+ "email=?, zipcode=?, address=?, hobby=?, job=? , nation=? where id=?";
			pstmt = con.prepareStatement(sql);
